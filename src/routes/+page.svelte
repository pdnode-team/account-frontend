    ...
    realtime.subscribe("account", response => {
      if (response.events.includes("users.*.update.verification")) {
        ...
        if (unverifiedEmailModal) {
          if (response.payload.emailVerification) unverifiedEmailModal.close(); else unverifiedEmailModal.showModal();
        }
      }
    })
  })
...
<main class="min-h-screen bg-base-200/50">
  ...
  <dialog id="showUnverifiedEmail" class="modal" bind:this={unverifiedEmailModal}>
    <div class="modal-box flex flex-col items-center">
      ...
      {#if verifyError}
        <div role="alert" class="alert alert-error alert-soft mb-4 w-full flex justify-center items-center">
          ...
          <div class="card bg-base-100 shadow-xl">
            <div class="card-body items-center text-center">
              ...
              <div class="flex flex-wrap justify-center gap-2">
                <span class="badge {user.emailVerification ? 'badge-success' : 'badge-warning badge-dash'}">{user.emailVerification ? 'Verified' : 'Unverified'}</span>
                <span class="badge badge-ghost">Joined on {new Date(user.$createdAt).toLocaleDateString()}</span>
              </div>

              <div class="divider"></div>

              <button class="btn btn-error btn-outline btn-block" onclick={handleLogout}>
                Logout
              </button>
            </div>
          </div>
        ...
        <div class="lg:col-span-8 space-y-6">
          <div class="card bg-base-100 shadow-xl border border-base-300">
            <div class="card-body">
              ...
              <div class="space-y-4">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-base-200/50 rounded-xl gap-4 transition hover:bg-base-200">
                  <div>
                    <p class="text-xs uppercase tracking-wider font-bold opacity-50">Email Address</p>
                    <p class="font-medium text-lg text-base-content">{user.email}</p>
                  </div>
                  <button class="btn btn-primary btn-sm px-6">Edit</button>
                </div>

                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-base-200/50 rounded-xl gap-4">
                  <div>
